 ...
 * visibility state and positions the slip with `className`, which MUST carry a
 * positioning class (`relative`, `absolute` or `fixed`): the base sets none, so
 * the accent tab's `before:absolute` anchors to whichever the consumer passes.
 * The toast calls `onDismiss` after `duration` ms.
 */
export function Toast({
  ...
  return (
    <div
      role="status"
      className={cx(
        "stock-textured flex items-center gap-3 overflow-hidden rounded-input bg-kraft py-2 pr-3 pl-4 text-ui text-ink shadow-lift",
